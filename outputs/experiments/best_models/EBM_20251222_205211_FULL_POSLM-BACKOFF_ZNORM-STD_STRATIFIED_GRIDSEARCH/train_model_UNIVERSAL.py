...
RESULTS_BASE = pathlib.Path(os.environ.get("RESULTS_BASE", 
    str(PROJECT_BASE / "outputs" / "experiments" / "resultados_modelos")))
...
def plot_confusion_matrix(cm, title, out_png):
    ...
    ax.set(xticks=np.arange(cm.shape[1]), yticks=np.arange(cm.shape[0]),
           ...
           xlabel='Severidad Real', ylabel='Severidad Predicha')
...
def main():
    ...
    parser.add_argument('--model', type=str, default='svr',
                   choices=['svr', 'lgbm', 'xgb', 'catboost', 'rf', 'elasticnet', 'tabpfn',
                           'ngboost', 'ebm', 'tabnet', 'symreg'],
                   help='Modelo: svr, lgbm, xgb, catboost, rf, elasticnet, tabpfn, ngboost, ebm, tabnet, symreg')
