...
def get_poslm_features(df, method='kneser-ney'):
    """
    Devuelve la lista de columnas POS-LM según el método elegido.
    Si method == 'all', devuelve todas las columnas que empiezan por 'poslm_'.
    Si method == 'kneser-ney' / 'backoff' / 'lstm', filtra por el prefijo correspondiente.
    """
    all_poslm = [c for c in df.columns if c.startswith('poslm_')]
    if not all_poslm:
        return []
    
    if method not in POSLM_METHODS or method == 'all':
        return sorted(all_poslm)
    
    prefix = POSLM_METHODS[method]
    return sorted([c for c in all_poslm if c.startswith(prefix)])
...
def main():
    ...
    parser.add_argument('--poslm-method', type=str, default='none',
                       ...
                       help='Método POS-LM a usar (none = sin POS-LM)')
    ...
    log.info("POS-LM method: {}".format(args.poslm_method))
    ...
    poslm_cols = []
    if args.poslm_method is not None and args.poslm_method != 'none':
        poslm_cols = get_poslm_features(df, method=args.poslm_method)
        if len(poslm_cols) > 0:
            log.info("\nPOS-LM:")
            log.info("  Metodo: {}".format(args.poslm_method))
            log.info("  Columnas POS-LM disponibles: {}".format(len(poslm_cols)))
            all_feat_cols = sorted(all_feat_cols + poslm_cols)
        else:
            log.warning("\nNo se han encontrado columnas POS-LM para el metodo '{}'".format(args.poslm_method))
    ...
    poslm_feat_cols = [f for f in feat_cols if f.startswith('poslm_')]
    
    log.info("\nFeatures:")
    log.info("  DEN:   {}".format(len(den_cols)))
    log.info("  DYS:   {}".format(len(dys_cols)))
    log.info("  LEX:   {}".format(len(lex_cols)))
    log.info("  POSLM: {}".format(len(poslm_feat_cols)))
    ...
    with open(run_dir / "features_used.txt", "w", encoding="utf-8") as f:
        ...
        for c in lex_cols:
            f.write("  - {}\n".format(c))
        f.write("\nPOSLM ({}):\n".format(len(poslm_feat_cols)))
        for c in poslm_feat_cols:
            f.write("  - {}\n".format(c))
