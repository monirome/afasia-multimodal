...
# ======================== FEATURE SELECTION ========================
def perform_sfs(X_all, y_all, feat_cols, run_dir, log, use_znorm_controls, X_control_all, base_model, max_features=40):
    try:
    ...
    if use_znorm_controls and X_control_all is not None:
        ...
        sfs = SFS(base_model, k_features=(1, max_features), forward=True, floating=False,
...
def main():
    ...
    parser.add_argument('--features', type=str, default='full',
                       ...
                       help='Feature selection: simple, full, sfs')
    ...
    parser.add_argument('--cv-inner', type=int, default=5,
                       help='Folds para GridSearchCV interno')
    ...
    if args.features == 'simple':
        ...
        # === PRE-FILTRADO INTELIGENTE PARA ACELERAR SFS ===
        log.info("\n" + "="*70)
        log.info("PRE-FILTRADO DE FEATURES PARA SFS")
        log.info("="*70)
        ...
        # 1. Eliminar features con >30% NaN
        ...
        # 2. Imputar y eliminar varianza casi cero
        ...
        all_feat_cols_filtered = [all_feat_cols_filtered[i] for i in range(len(all_feat_cols_filtered)) if variance_mask[i]]
        
        # 3. SelectKBest: quedarnos con top 80 features mas correlacionadas
        if len(all_feat_cols_filtered) > 80:
            log.info(f"Paso 3: Reduciendo de {len(all_feat_cols_filtered)} a 80 features con SelectKBest (f_regression)...")
            ...
            all_feat_cols = [all_feat_cols_filtered[i] for i in range(len(all_feat_cols_filtered)) if kbest_mask[i]]
            log.info(f"Resultado: {len(all_feat_cols)} features para SFS")
        else:
            all_feat_cols = all_feat_cols_filtered
            log.info(f"Resultado: {len(all_feat_cols)} features para SFS (no se necesito reducir)")
        
        # Guardar features pre-filtradas
        with open(run_dir / "prefiltered_features_sfs.txt", "w") as f:
            f.write("FEATURES PRE-FILTRADAS PARA SFS\n")
            f.write("="*70 + "\n")
            f.write(f"Total: {len(all_feat_cols)}\n\n")
            for feat in sorted(all_feat_cols):
                f.write(f"{feat}\n")
        
        # Reconstruir datos con features filtradas
        X_pwa_en_all = df_en[all_feat_cols].values
        y_pwa_en_all = df_en['QA'].values
        ...
        # === MODELO OPTIMIZADO PARA SFS ===
        log.info("\n" + "="*70)
        log.info("CONFIGURACION DE MODELO PARA SFS")
        log.info("="*70)
        
        if args.model.lower() == "catboost":
            log.info("Usando SVR para SFS (CatBoost entrena despues)")
            sfs_base_model = SVR(kernel="rbf", C=10.0, epsilon=0.1)
        
        elif args.model.lower() == "lgbm":
            from lightgbm import LGBMRegressor
            
            log.info("Usando LightGBM con parametros fijos optimizados para SFS")
            log.info("  num_leaves=31, learning_rate=0.1, n_estimators=100")
            log.info("  Esto es ~8x mas rapido que grid search completo")
            ...
            log.info("Usando XGBoost con parametros fijos optimizados para SFS")
            log.info("  max_depth=5, learning_rate=0.1, n_estimators=100")
            log.info("  Esto es ~8x mas rapido que grid search completo")
            ...
            log.info(f"Usando {args.model.upper()} original para SFS")
            sfs_base_model = clone(base_model)
        
        # Ejecutar SFS con max_features reducido
        selected_idx, feat_cols = perform_sfs(
            X_all,
            y_all,
            all_feat_cols,
            run_dir,
            ...
            X_control_all,
            sfs_base_model,
            max_features=20  # REDUCIDO de 40 a 20
        )
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(split_generator, 1):
        ...
        log.info("    Best params: {}".format(gs_fold.best_params_))
        
        fold_result = {'fold': fold_idx, 'mae': mae_fold}
        fold_result.update(gs_fold.best_params_)
    ...
    if args.znorm_controls and n_control_final > 0:
        final_model_dict = {
            ...
            'znorm_std': std_final,
            'model': gs_final.best_estimator_,
    ...
    exp_config = create_experiment_config(args, param_grid_logger, df_info, feat_cols)
