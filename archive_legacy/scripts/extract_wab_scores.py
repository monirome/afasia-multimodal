...
OUTPUT_VALID = "../data/patient_wab_scores.csv"
OUTPUT_NO_WAB = "../data/patients_no_wab.csv"
...
def main():
    ...
    if len(results) > 0:
        ...
        df.to_csv(OUTPUT_VALID, index=False)
        print(f"\n✓ Guardado en: {OUTPUT_VALID}")
    ...
    if len(no_score) > 0:
        pd.DataFrame(no_score).to_csv(OUTPUT_NO_WAB, index=False)
        print(f"\nPacientes sin score guardados en: {OUTPUT_NO_WAB}")
